...
class _MapMarkersState extends State<MapMarkers>
  ...
  Color _colorFor(String riskLevel) {
    switch (riskLevel.toUpperCase().trim()) {
      case 'EXTREME':
        return const Color(0xFFE040FB); // magenta — above HFL
      case 'CRITICAL':
      case 'HIGH':
        return const Color(0xFFFF4D5A); // core danger red
      case 'DANGER':
      case 'SEVERE':
        return const Color(0xFFFF8C42); // core severe orange
      case 'WARNING':
      case 'MODERATE':
        return const Color(0xFFFFC857); // core warning amber
      case 'LOW':
      case 'NORMAL':
        return const Color(0xFF3ACC8A); // core success green
      default:
        return const Color(0xFF7A8290); // core textMuted grey
    }
  }
  ...
  double _amplitudeFor(String riskLevel) {
    switch (riskLevel.toUpperCase().trim()) {
      ...
      case 'MODERATE':
        return 1.0;
      default:
        return 0.0; // no pulse for NORMAL
    }
  }
                  ...
                  content: Column(
                    ...
                    children: [
                      ...
                      LinearProgressIndicator(
                        value: station.dangerLevel != null &&
                  ...
                  actions: [
                    TextButton(
                      ...
                      child: const Text('Close',
                          style: TextStyle(color: Color(0xFF4CB3FF))),
                    ),
                  ],
                ),
              );
...
class _PulseMarkerPainter extends CustomPainter {
  ...
  bool shouldRepaint(covariant _PulseMarkerPainter old) =>
      old.animValue != animValue || old.color != color || old.radius != radius;
}
...
class _LevelRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        Text(label,
            style: const TextStyle(color: Color(0xFF7A8290), fontSize: 12)),
        Text(value,
            style: TextStyle(
