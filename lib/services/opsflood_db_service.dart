...
class OpsFloodDbService {
  ...
  String _col(String name) => _prefix.isEmpty ? name : '${_prefix}_$name';
  ...
  Future<DbResult<DbStation>> getStations() async {
    ...
    if (cached.value != null) {
      final list =
          (jsonDecode(cached.value!) as List).cast<Map<String, dynamic>>();
      return DbResult(
          data: list.map(DbStation.fromMap).toList(),
          fromCache: true,
    ...
    if (fs != null) {
      try {
        final snap =
            await fs.collection(_col('station_registry')).limit(200).get();
        final result =
            snap.docs.map((d) => DbStation.fromMap(d.data())).toList();
        if (result.isNotEmpty)
          return DbResult(data: result, fromCache: true, isStale: true);
      } catch (e) {
        _log('Firestore station fallback: $e');
      }
    }
  ...
  Future<DbResult<DbReading>> getAllReadings() async {
    ...
    if (cached.value != null) {
      final list =
          (jsonDecode(cached.value!) as List).cast<Map<String, dynamic>>();
      return DbResult(
          data: list.map(DbReading.fromMap).toList(),
          fromCache: true,
          isStale: cached.isStale);
    }
    return DbResult(data: const [], error: raw['error']?.toString());
  ...
  Future<DbResult<DbAlert>> getCriticalAlerts() async {
    ...
    if (cached.value != null) {
      final list =
          (jsonDecode(cached.value!) as List).cast<Map<String, dynamic>>();
      return DbResult(
          data: list.map(DbAlert.fromMap).toList(),
          fromCache: true,
          isStale: cached.isStale);
    }
    return DbResult(data: const [], error: raw['error']?.toString());
  ...
  Future<DbResult<DbPrediction>> getPredictions({String? city}) async {
    ...
    if (_isOk(raw)) {
      final obj = raw.containsKey('data') && raw['data'] is Map
          ...
          : raw;
      final result =
          DbPrediction.fromMap({...obj, if (city != null) 'city': city});
      if (city != null) {
        await _cache.setString(
            '$path?city=${Uri.encodeComponent(city)}', jsonEncode([obj]));
      }
  ...
  void _mirrorAlerts(List<DbAlert> alerts) {
    ...
    for (final a in alerts) {
      ...
      final docId =
          '${a.city.toLowerCase().replaceAll(' ', '_')}_${a.timestamp.millisecondsSinceEpoch}';
      col
          .doc(docId)
  ...
  void _mirrorPrediction(DbPrediction p) {
    ...
    final docId =
        '${p.city.toLowerCase().replaceAll(' ', '_')}_${p.generatedAt.toUtc().toIso8601String().substring(0, 10)}';
    col
        .doc(docId)
