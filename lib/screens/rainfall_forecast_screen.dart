...
// ── Search provider ──────────────────────────────────────────────────────────
final _searchProvider = NotifierProvider<_SearchNotifier, String>(_SearchNotifier.new);
    ...
    return Scaffold(
        ...
        bottom: PreferredSize(
          ...
          child: Column(children: [
            ...
            Padding(
              padding: const EdgeInsets.fromLTRB(16, 0, 16, 8),
              child: Row(children: [
                _HorizonTab(label: '24 Hours', active: horizon == 0, color: c.accent, onTap: () => ref.read(_horizonProvider.notifier).state = 0),
                const SizedBox(width: 8),
                _HorizonTab(label: '48 Hours', active: horizon == 1, color: c.accent, onTap: () => ref.read(_horizonProvider.notifier).state = 1),
                const SizedBox(width: 8),
                _HorizonTab(label: '72 Hours', active: horizon == 2, color: c.accent, onTap: () => ref.read(_horizonProvider.notifier).state = 2),
                const Spacer(),
                Text('\${preds.length} stations', style: TextStyle(color: c.textMuted, fontSize: 11)),
              ]),
            ),
          ]),
        ),
      ...
      body: preds.isEmpty
          ...
          : ListView.separated(
              ...
              itemBuilder: (_, i) => _StationCard(pred: preds[i], horizon: horizon, c: c),
            ),
    );
  }
}
...
class _HorizonTab extends StatelessWidget {
  ...
  const _HorizonTab({required this.label, required this.active, required this.color, required this.onTap});
...
class _StationCard extends StatelessWidget {
  ...
  final int horizon;
  final dynamic c;
  const _StationCard({required this.pred, required this.horizon, required this.c});
  ...
  double get _level {
    switch (horizon) {
      case 0:  return pred.predicted24h;
      case 1:  return pred.predicted48h;
      default: return pred.predicted72h;
    }
  }

  String get _horizonLabel {
    switch (horizon) {
      case 0:  return 'in 24h';
      case 1:  return 'in 48h';
      default: return 'in 72h';
    }
  }
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(name, style: TextStyle(color: c.textPrimary, fontSize: 14, fontWeight: FontWeight.w700)),
              Text(pred.river, style: TextStyle(color: c.textSecondary, fontSize: 11)),
            ])),
          ...
          Row(children: [
            ...
            Text(_horizonLabel, style: TextStyle(color: c.textMuted, fontSize: 11)),
            const Spacer(),
            Row(children: [
              _MiniLevel(label: '24h', value: pred.predicted24h, active: horizon == 0, color: color, c: c),
              const SizedBox(width: 4),
              _MiniLevel(label: '48h', value: pred.predicted48h, active: horizon == 1, color: color, c: c),
              const SizedBox(width: 4),
              _MiniLevel(label: '72h', value: pred.predicted72h, active: horizon == 2, color: color, c: c),
            ]),
          ]),
...
class _MiniLevel extends StatelessWidget {
  ...
  final Color color;
  final dynamic c;
  const _MiniLevel({required this.label, required this.value, required this.active, required this.color, required this.c});
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 12),
        Text(query.isEmpty ? 'No forecast data available' : 'No stations match "$query"',
