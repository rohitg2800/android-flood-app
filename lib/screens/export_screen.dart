...
class _ExportScreenState
      ...
      body: ListView(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                Icon(Icons.sensors_rounded, color: t.accent, size: 16),
                const SizedBox(width: 8),
                Text(
          ...
          _ExportActionCard(
            ...
            title:       'Export as PDF',
            subtitle:    'A4 landscape • Severity colour coding • Multi-page',
  ...
  Future<void> _exportCsv() async {
    ...
    try {
      final path = await ExportService.instance.exportAndShareCsv(_rows);
      if (mounted) setState(() => _lastFilePath = path);
    } catch (e) {
      if (context.mounted) _showError('CSV export failed: $e');
    } finally {
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      final path = await ExportService.instance.exportAndSharePdf(
        _rows,
        reportTitle:
...
class _ExportActionCard extends StatelessWidget {
  ...
  const _ExportActionCard({
    required this.t, required this.icon, required this.iconColor,
    required this.title, required this.subtitle,
    required this.buttonLabel, required this.loading, required this.onTap,
  });
  @override
  Widget build(BuildContext context) => Container(
        padding: const EdgeInsets.all(16),
