...
({double warning, double danger, double hfl, String river})? _lookupThreshold(
    String normName) {
...
class LiveEngineBridgeNotifier extends Notifier<List<RiverStation>> {
  ...
  void _onFeed(BiharLiveFeed feed) {
    ...
    if (kDebugMode) {
      debugPrint(
          '[LiveEngineBridge] ${state.length} stations from engine feed');
    }
  }
  ...
  List<RiverStation> _convert(BiharLiveFeed feed) {
    ...
    for (final item in feed.items) {
      ...
      result.add(RiverStation(
        ...
        danger: danger,
        hfl: hfl,
        lastUpdated: '${item.fetchedAt.hour.toString().padLeft(2, '0')}:'
