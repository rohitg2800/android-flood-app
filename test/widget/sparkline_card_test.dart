...
void main() {
  ...
  testWidgets('shows spinner while loading', (tester) async {
    ...
    await tester.pumpWidget(_wrap(const SparklineCard(
      stationId: 'GS001', dangerLevel: 10.0, accentColor: Colors.blue,
    )));
  ...
  testWidgets('passes correct stationId to cache', (tester) async {
    String? got;
    fake.onLoad = (id) async { got = id; return <(DateTime, double)>[]; };
    await tester.pumpWidget(_wrap(const SparklineCard(
      stationId: 'GS_PATNA', dangerLevel: 12.0, accentColor: Colors.red,
    )));
    await tester.pumpAndSettle();
    expect(got, 'GS_PATNA');
  });
